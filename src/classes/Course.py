...
class Course:
    def __init__(self, name : str, raw_data: List[num], normalize: bool = False):
        ...
        self.raw_data_with_no_nan = self.sort_data(False)
        # Sorting the raw data
        self.data_sorted : List[num] = self.sort_data(True)
        # Compute new count
        ...
        if normalize :
            self.data_zscore = self.normalize_data(self.raw_data_with_no_nan)
    ...
    def sort_data(self, sorted:bool) -> np.ndarray:
